...
$pdf->SetFont('','B');
    // Header
$w = array(7,30,40,30,25,25,35);
	$header = array('#','NAME','DEPARTMENT','LEAVE','START','END ','DATE APPLIED');
...
$leave=Leave::find_by_leave_id($apps->leave_id);
    ...
    $pdf->Cell($w[2],5.5,$dept->department_name,'LR',0,'L',$fill);
...
else if(isset($_POST['report4'])){
   $pdf->Cell(0,10,'report 4',0,1,'A');
...
$pdf->SetFont('','B');
    // Header
$w = array(7,40, 45, 30, 25, 25);
	$header = array('#','NAME','DEPARTMENT','TERMS','SEX','ACCOUNT');
...
$dept=Department::find_by_department_id($staff->department_id);
	...
	$pdf->Cell($w[1],5.5,$staff->first_name ." ". $staff->last_name,'LR',0,'L',$fill);
    $pdf->Cell($w[2],5.5,$dept->department_name,'LR',0,'L',$fill);
    $pdf->Cell($w[3],5.5,$staff->contract_type,'LR',0,'LR',$fill);
	$pdf->Cell($w[4],5.5,$staff->gender,'LR',0,'R',$fill);
	$pdf->Cell($w[4],5.5,$staff->balance,'LR',0,'R',$fill);
    $pdf->Ln();	
	$fill = !$fill;	
    // Closing line
...
$manager=Staff::manager_department($_POST['department']);
$dept=Department::find_by_department_id($_POST['department']);
$staff=Staff::find_staff_by_department($_POST['department']);
$pdf->Cell(0,10,'DEPARTMENT: '. $dept->department_name,0,1,'A');
if($manager){
$pdf->Cell(0,10,'MANAGER: '. $manager->first_name ." ".$manager->last_name,0,1,'A');
}
...
$pdf->SetFont('','B');
    // Header
$w = array(7,40,40,40,40);
	$header = array('#','NAME', 'TERMS', 'SEX','ACCOUNT');
